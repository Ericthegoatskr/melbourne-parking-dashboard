 ...
 */
function colourFor(street) {
    if (street.vacancyRate === null)
        return '#64748b';
    if (street.vacancyRate >= 0.3)
        return '#34d399';
    if (street.vacancyRate >= 0.12)
        return '#fbbf24';
    return '#fb7185';
}
export function ParkingMap({ streets, state, selected, onSelect }) {
    ...
    useEffect(() => {
        ...
        for (const street of streets) {
            ...
            marker.bindPopup(`<strong>${escapeHtml(street.street)}</strong><br/>` +
                (silent
    ...
    return (_jsx("section", { className: "card h-full p-0", children: _jsx("div", { ref: containerRef, className: "h-[420px] w-full overflow-hidden rounded-xl lg:h-full lg:min-h-[420px]", role: "application", "aria-label": "Map of on-street parking availability by street" }) }));
}
