...
import {
  UNMAPPED_STREET,
  describeRestriction,
  describeStreetSignage,
  filterStreets,
  groupByStreet,
  sortStreets,
  summarise,
} from '../lib/parkingModel';
import type { Bay, Snapshot, ZoneCrosswalk } from '../lib/types';

const CROSSWALK: ZoneCrosswalk = {
  '7001': {
...
function snapshotOf(bays: Bay[]): Snapshot {
  return {
    bays,
...
describe('street grouping', () => {
  ...
  it('refuses to name one restriction when the signage disagrees', () => {
    ...
    expect(describeStreetSignage(streets[0]!)).toBe(
      'Metered 2hr / Loading zone 30min — read the sign',
    );
  });
...
describe('street ranking', () => {
  ...
  it('never puts the unmapped bookkeeping group at the top of a recommendation', () => {
    ...
    for (const mode of ['most-vacant', 'best-odds'] as const) {
