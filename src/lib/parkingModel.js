 ...
 */
export function groupByStreet(bays, crosswalk) {
    ...
    for (const bay of bays) {
        ...
        if (bay.zone !== null)
            bucket.zones.add(bay.zone);
        bucket.total += 1;
        bucket.latSum += bay.lat;
        bucket.lonSum += bay.lon;
        if (!bay.trusted)
            bucket.notReporting += 1;
        else if (bay.occupied)
            bucket.occupied += 1;
        else
            bucket.vacant += 1;
    ...
    return [...buckets.values()].map((bucket) => {
        ...
        const codes = [...bucket.codes.entries()].map(([code, days]) => ({
            code,
            days,
        }));
        const restriction = codes.length === 1 ? codes[0] : null;
        return {
 ...
 */
export function sortStreets(streets, mode, minReporting = 5) {
    ...
    return sorted.sort((a, b) => {
        ...
        if (bReporting === 0)
            return -1;
        if (mode === 'most-vacant')
            return b.vacant - a.vacant || a.street.localeCompare(b.street);
...
export function describeRestriction(code) {
    if (!code)
        return null;
 ...
 */
export function describeStreetSignage(street) {
...
export function filterStreets(streets, query) {
