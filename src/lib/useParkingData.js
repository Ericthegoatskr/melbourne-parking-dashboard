...
export function useParkingData() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            if (previous && fresh.sourceTime < previous.sourceTime) {
                ...
                setRejection(new SnapshotRejected('stale-regression', 'Ignored a feed response older than the data already shown.', `Received ${fresh.sourceTime.toISOString()}, holding ${previous.sourceTime.toISOString()}.`));
                return;
            }
            ...
            setRejection(error instanceof SnapshotRejected
                ? error
     ...
     */
    const state = snapshot ? classifySnapshot(snapshot, new Date()) : 'unavailable';
    return {
        snapshot,
        state,
        rejection,
