import {
  ...
  OCCUPIED_STATUS,
  REQUEST_TIMEOUT_MS,
  SENSOR_TRUST_WINDOW_MINUTES,
  VALIDATION,
} from './config';
import { SnapshotRejected } from './types';
import type { Bay, Snapshot } from './types';
...
function parseCoord(location: unknown): { lat: number; lon: number } | null {
  ...
  if (lat > -37.5 || lat < -38.2) return null;
  if (lon < 144.5 || lon > 145.4) return null;
  return { lat, lon };
}
 ...
 */
export function validatePayload(payload: unknown, now: Date): Snapshot {
  ...
  if (payload.length < VALIDATION.minRecords) {
    throw new SnapshotRejected(
      'too-few-records',
      'The feed returned far fewer bays than the sensor network contains.',
      `${payload.length} records, expected at least ${VALIDATION.minRecords}.`,
    );
  }

  const parsed: Omit<Bay, 'trusted'>[] = [];
  let unparseable = 0;
  for (const raw of payload as RawBay[]) {
    const bay = parseBay(raw);
    if (bay) parsed.push(bay);
    else unparseable += 1;
  }
  ...
  if (unparseableRatio > VALIDATION.maxUnparseableRatio) {
    throw new SnapshotRejected(
      'unparseable-records',
      'Too much of the feed could not be read.',
      `${(unparseableRatio * 100).toFixed(1)}% of records were unreadable.`,
    );
  }
  if (parsed.length === 0) {
  ...
  for (const bay of parsed) {
    if (bay.reportedAt > sourceTime) sourceTime = bay.reportedAt;
  }
  ...
  if (skewMinutes > VALIDATION.maxFutureSkewMinutes) {
    throw new SnapshotRejected(
      'future-source-time',
      'The feed reports a time in the future.',
      `Source time is ${skewMinutes.toFixed(1)} minutes ahead of this device.`,
    );
  }
  ...
  if (trustedRatio < VALIDATION.minTrustedRatio) {
    throw new SnapshotRejected(
      ...
      'Too few sensors are currently reporting to describe the city.',
      `${(trustedRatio * 100).toFixed(1)}% reporting, need ${(
        VALIDATION.minTrustedRatio * 100
      ).toFixed(0)}%.`,
    );
  }
  ...
  if (
    occupancy < VALIDATION.occupancyBounds.min ||
    occupancy > VALIDATION.occupancyBounds.max
  ) {
    throw new SnapshotRejected(
      'impossible-occupancy',
      'Occupancy figures are not internally consistent.',
      `Computed occupancy ${occupancy}.`,
    );
  }
...
export async function fetchSnapshot(
  ...
  if (!response.ok) {
    throw new SnapshotRejected(
      'http',
      'The open-data feed returned an error.',
